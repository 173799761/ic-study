import { course_5, idlFactory, canisterId as main_canister_id } from "../../declarations/mcn";
...
function App() {
  ...
  const getData = async () => {
    try {
      let team = await course_5.get_owner_list();
      setTeam(team);
  
      let proposals = await course_5.get_proposals();
      proposals.sort((x,y) => {
        let m = parseInt(x.id);
        let n = parseInt(y.id);
        if (m < n) return 1;
      ...
      let r = await course_5.get_model();
      setM(r[0].toString());
      setN(r[1].toString());
  
      let canisters = await course_5.get_owned_canisters_list();
      let canistersM = new Array(canisters.length);
      let canistersStatus = new Array(canisters.length);
      for(var i = 0; i < canisters.length; i++) {
        canistersM[i] = await course_5.get_permission(canisters[i]);
        canistersStatus[i] = await course_5.get_status(canisters[i]);
      }
